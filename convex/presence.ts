// convex/presence.ts
import { Presence } from "@convex-dev/presence";
import { ConvexError, v } from "convex/values";
import { components } from "./_generated/api";
import { mutation, query } from "./_generated/server";
import { authComponent } from "./auth";

export const presence = new Presence(components.presence);

export const heartbeat = mutation({
  args: {
    roomId: v.string(),
    userId: v.string(), // ✅ REQUIRED by usePresence hook
    sessionId: v.string(),
    interval: v.number(),
  },
  handler: async (ctx, { roomId, userId, sessionId, interval }) => {
    // ✅ Verify the userId matches the authenticated user
    const user = await authComponent.safeGetAuthUser(ctx);

    if (!user) {
      throw new ConvexError("Unauthorized: Not authenticated");
    }

    // ✅ Verify the userId parameter matches the authenticated user
    if (user._id !== userId) {
      throw new ConvexError("Unauthorized: User ID mismatch");
    }

    return await presence.heartbeat(ctx, roomId, userId, sessionId, interval);
  },
});

export const list = query({
  args: { roomToken: v.string() },
  handler: async (ctx, { roomToken }) => {
    const entries = await presence.list(ctx, roomToken);

    // Fetch users using Better Auth
    const usersData = await Promise.all(
      entries.map(async (entry) => {
        try {
          // ✅ entry.userId is a string ID
          const user = await authComponent.getAnyUserById(ctx, entry.userId);
          return user;
        } catch (error) {
          console.error(`Failed to fetch user ${entry.userId}:`, error);
          return null;
        }
      })
    );

    return entries.map((entry, index) => {
      const user = usersData[index];
      return {
        ...entry,
        name: user?.name ?? "Anonymous",
      };
    });
  },
});

export const disconnect = mutation({
  args: { sessionToken: v.string() },
  handler: async (ctx, { sessionToken }) => {
    return await presence.disconnect(ctx, sessionToken);
  },
});

export const getUserId = query({
  args: {},
  handler: async (ctx) => {
    const user = await authComponent.safeGetAuthUser(ctx);
    return user?._id ?? null;
  },
});
